...
import type {
  FinanceCustomerStatementLineRow,
  FinanceCustomerStatementRow,
} from "@backend/shared/types/finance-customer-statement";

export const TAX_INVOICE_CSV_HEADERS = [
  "customer_statement_number",
  "statement_date",
  "valid_until_date",
  "client_id",
  "sales_order_id",
...
function linesToJson(lines: FinanceCustomerStatementLineRow[] | undefined): string {
...
export function taxInvoiceRowToCsvRecord(
  ...
  return {
    customer_statement_number: r.customer_statement_number ?? "",
    statement_date: r.statement_date ?? "",
    valid_until_date: r.valid_until_date ?? "",
    client_id: r.client_id ?? "",
    sales_order_id: r.sales_order_id ?? "",
    invoice_type: r.invoice_type ?? "service",
    customer_statement_status: r.customer_statement_status ?? "pending",
    notes: r.notes ?? "",
    terms_and_conditions: r.terms_and_conditions ?? "",
    scope_of_work: r.scope_of_work ?? "",
    bank_details: r.bank_details ?? "",
    seal_and_sign: r.seal_and_sign ?? "",
    lines_json: linesToJson(r.finance_customer_statement_lines),
  };
}

export function buildFinanceCustomerStatementsExportCsv(
...
export function parseFinanceCustomerStatementsImportCsv(text: string):
  ...
  if (!indexByKey.has("statement_date") || !indexByKey.has("valid_until_date")) {
    return {
      ok: false,
      error:
        "CSV must include statement_date and valid_until_date columns (use Export as a template).",
    };
  }
