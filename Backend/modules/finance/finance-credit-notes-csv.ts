...
import type {
  FinanceCreditNoteLineRow,
  FinanceCreditNoteRow,
} from "@backend/shared/types/finance-credit-note";
...
function linesToJson(lines: FinanceCreditNoteLineRow[] | undefined): string {
...
export function taxInvoiceRowToCsvRecord(
  ...
  return {
    ...
    credit_note_status: r.credit_note_status ?? "pending",
    notes: r.notes ?? "",
    terms_and_conditions: r.terms_and_conditions ?? "",
    scope_of_work: r.scope_of_work ?? "",
    bank_details: r.bank_details ?? "",
    seal_and_sign: r.seal_and_sign ?? "",
    lines_json: linesToJson(r.finance_credit_note_lines),
  };
}

export function buildFinanceCreditNotesExportCsv(
...
export function parseFinanceCreditNotesImportCsv(text: string):
  ...
  if (!indexByKey.has("credit_note_date") || !indexByKey.has("valid_until_date")) {
    return {
      ok: false,
      error:
        "CSV must include credit_note_date and valid_until_date columns (use Export as a template).",
    };
  }
