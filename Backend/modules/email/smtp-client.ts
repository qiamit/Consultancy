...
export async function sendEmail(
  ...
  // Prefer Resend for verified project domains (e.g. @qengineering.in).
  if (canSendViaResend(account.email_address)) {
    return sendEmailViaResend(account, input);
  }
...
export async function testSmtpConnection(account: EmailAccountRow): Promise<void> {
  if (canSendViaResend(account.email_address)) {
    const { verifyResendApiKey } = await import("@backend/modules/email/resend");
