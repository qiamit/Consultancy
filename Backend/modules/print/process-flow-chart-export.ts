...
import type { ProcessFlowChartLetterData } from "@backend/modules/print/process-flow-chart";
...
async function buildProcessFlowChartDocx(data: ProcessFlowChartLetterData): Promise<Document> {
  ...
  const children: Paragraph[] = [
    new Paragraph({
      ...
      spacing: { after: 200 },
      children: [bodyRun("Process Flow Chart", true)],
    }),
    ...
    plainParagraph(
      data.document.drawing_data_url
        ? "Process flow chart is attached in the application print preview."
        : "Process flow chart is not yet added.",
    ),
    plainParagraph(
...
export async function downloadProcessFlowChartWord(
  ...
  const docx = await buildProcessFlowChartDocx(data);
