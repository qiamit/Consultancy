...
import { joinProformaInvoiceNumberParts } from "@backend/modules/finance/finance-proforma-invoice-number";
import { createClient } from "@backend/db/client/server";

const LIST_PATH = "/dashboard/finance/sales/proforma-invoice";
...
export async function saveFinanceProformaInvoice(formData: FormData) {
  ...
  const proforma_date = str(formData, "proforma_date");
  ...
  const proforma_status_in = str(formData, "proforma_status").toLowerCase();
  const proforma_status: ProformaStatus | null =
    proforma_status_in === "accepted" || proforma_status_in === "cancelled"
      ? proforma_status_in
      : proforma_status_in === "pending"
        ? "pending"
        : null;
  if (invoice_type !== "service" && invoice_type !== "supply") {
    redirect(`${LIST_PATH}?error=type`);
  }
  if (!proforma_date || !valid_until_date) redirect(`${LIST_PATH}?error=dates`);
  ...
  const proforma_invoice_number_in = joinProformaInvoiceNumberParts(
    str(formData, "proforma_invoice_number_prefix"),
    str(formData, "proforma_invoice_number_value"),
  );
  ...
  if (id) {
    if (!proforma_invoice_number_in) {
      redirect(`${LIST_PATH}?error=proforma_invoice_number_required`);
    }
    const { error: uErr } = await supabase
      .from("finance_proforma_invoices")
      .update({
        ...header,
        proforma_invoice_number: proforma_invoice_number_in,
      })
      .eq("id", id);
    if (uErr?.code === "23505") {
      redirect(`${LIST_PATH}?error=proforma_invoice_number_duplicate`);
    }
    if (uErr) redirect(`${LIST_PATH}?error=db`);
    const { error: dErr } = await supabase
      .from("finance_proforma_invoice_lines")
      .delete()
      .eq("proforma_invoice_id", id);
    if (dErr) redirect(`${LIST_PATH}?error=db`);
    const rows = built.map((b) => ({ ...b, proforma_invoice_id: id }));
  ...
  const rows = built.map((b) => ({ ...b, proforma_invoice_id: newId }));
  const { error: lErr } = await supabase
    .from("finance_proforma_invoice_lines")
    .insert(rows);
  if (lErr) redirect(`${LIST_PATH}?error=db`);
...
export async function deleteFinanceProformaInvoice(id: string) {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
...
export async function deleteFinanceProformaInvoiceForm(formData: FormData) {
  await deleteFinanceProformaInvoice(String(formData.get("id") ?? ""));
}

export async function deleteFinanceProformaInvoices(ids: string[]) {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
...
export async function importFinanceProformaInvoices(
  ...
  for (let i = 0; i < records.length; i++) {
    const rec = records[i];
    const proforma_date = String(rec.proforma_date ?? "").trim();
    const valid_until_date = String(rec.valid_until_date ?? "").trim();
    if (!proforma_date || !valid_until_date) {
      return {
        ok: false,
        error: `Row ${i + 2} (after header): proforma_date and valid_until_date are required.`,
      };
    }
    ...
    const header = {
      ...
      invoice_type,
      proforma_status:
        String(rec.proforma_status ?? "").trim().toLowerCase() === "accepted" ||
        String(rec.proforma_status ?? "").trim().toLowerCase() === "cancelled"
          ? (String(rec.proforma_status ?? "").trim().toLowerCase() as ProformaStatus)
          : "pending",
    ...
    const num = String(rec.proforma_invoice_number ?? "").trim();
    const insertHeader = {
      ...header,
      proforma_invoice_number: num,
      created_by: user.id,
    };
    ...
    if (iErr || !ins?.id) {
      if (iErr?.code === "23505") {
        return {
          ok: false,
          error: `Row ${i + 2} (after header): duplicate proforma_invoice_number or database constraint.`,
        };
      }
      return {
        ok: false,
    ...
    const newId = ins.id as string;
    const lineRows = built.map((b) => ({ ...b, proforma_invoice_id: newId }));
    const { error: lErr } = await supabase
      .from("finance_proforma_invoice_lines")
      .insert(lineRows);
    if (lErr) {
      await supabase.from("finance_proforma_invoices").delete().eq("id", newId);
      return {
        ok: false,
...
export async function updateFinanceProformaInvoiceStatus(input: {
  id: string;
  ...
  if (!id) return { ok: false, error: "Proforma invoice id missing." };
  ...
  const { error } = await supabase
    ...
    .eq("id", id);
  if (error) return { ok: false, error: "Could not update proforma status." };
