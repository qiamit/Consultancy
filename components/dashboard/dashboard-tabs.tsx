...
function KpiCard({
  ...
  const inner = (
    <div className={`rounded-2xl border ${colors.border} ${colors.bg} p-5 flex flex-col gap-2 shadow-sm transition-all ${href ? "hover:shadow-md hover:scale-[1.01]" : ""}`}>
      <p className={`text-[11px] font-bold uppercase tracking-wider ${colors.text}`}>{label}</p>
      <p className={`text-4xl font-black tabular-nums ${colors.num}`}>{value}</p>
      {sub && <p className={`text-xs font-medium ${colors.text} opacity-80`}>{sub}</p>}
    </div>
  );
...
function GeneralTab({
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="mb-3 text-xs font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">Master Data</p>
      ...
      <div>
        <p className="mb-3 text-xs font-bold uppercase tracking-wider text-zinc-400 dark:text-zinc-500">BIS License Status</p>
