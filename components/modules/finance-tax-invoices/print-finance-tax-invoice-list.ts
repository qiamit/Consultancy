...
function listTableHtml(rows: FinanceTaxInvoiceRow[]): string {
  ...
  const body = rows
    .map(
      (r, i) =>
        `<tr><td>${i + 1}</td><td class="mono">${esc(r.tax_invoice_number)}</td><td>${esc(r.tax_date)}</td><td>${esc(r.valid_until_date)}</td><td class="mono">${esc(r.finance_sales_orders?.sales_order_number ?? "—")}</td><td>${esc(clientLabel(r))}</td><td>${esc(r.invoice_type)}</td><td class="num">${esc(
