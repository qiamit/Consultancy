import { FinanceTaxInvoicesMaster } from "./master";
...
export async function FinanceTaxInvoicesServer({
  searchParams,
}: {
  ...
  const [
    ...
    notesTemplates,
      ...
      supabase
        ...
        .select(
          `id, tax_invoice_number, tax_status, tax_date, valid_until_date, client_id, quotation_id, sales_order_id, proforma_invoice_id, invoice_type,
          ...
          finance_tax_invoice_lines(
            id, tax_invoice_id, sort_order, product_master_item_id, item_description,
  ...
  const rows = (qRaw ?? []) as unknown as FinanceTaxInvoiceRow[];
  for (const r of rows) {
    const lines = r.finance_tax_invoice_lines;
