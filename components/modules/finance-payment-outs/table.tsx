...
export function FinancePaymentOutsTable({
  rows,
  idParam,
  onEditRow,
  matchedCount,
  ...
  rows: FinancePaymentOutRow[];
  idParam: string | null;
  onEditRow: (r: FinancePaymentOutRow) => void;
  matchedCount: number;
  totalAmount: number;
}) {
  return (
    <div className="overflow-x-auto">
      <table className="min-w-[920px] w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-800">
        ...
        <FinancePaymentOutsFooterBar matchedCount={matchedCount} totalAmount={totalAmount} />
      </table>
    </div>
  );
}
