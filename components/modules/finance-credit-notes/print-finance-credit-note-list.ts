...
function listTableHtml(rows: FinanceCreditNoteRow[]): string {
  ...
  const body = rows
    .map(
      (r, i) =>
        `<tr><td>${i + 1}</td><td class="mono">${esc(r.credit_note_number)}</td><td>${esc(r.credit_note_date)}</td><td>${esc(r.valid_until_date)}</td><td class="mono">${esc(r.finance_sales_orders?.sales_order_number ?? "—")}</td><td>${esc(clientLabel(r))}</td><td>${esc(r.invoice_type)}</td><td class="num">${esc(
