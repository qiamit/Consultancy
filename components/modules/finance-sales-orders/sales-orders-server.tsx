import { FinanceSalesOrdersMaster } from "./master";
...
export async function FinanceSalesOrdersServer({
  searchParams,
}: {
  ...
  const [
    ...
    notesTemplates,
      ...
      supabase
        ...
        .select(
          `id, sales_order_number, order_status, order_date, expected_delivery_date, client_id, quotation_id, order_type,
  ...
  const rows = (qRaw ?? []) as unknown as FinanceSalesOrderRow[];
  for (const r of rows) {
  ...
  const quotationIdPrefill = firstSearchParam(sp, "quotation_id");
  const wantQuotationPrefill =
    quotationIdPrefill && firstSearchParam(sp, "new") === "1";

  let prefillFromQuotation: FinanceQuotationRow | null = null;
  if (wantQuotationPrefill && quotationIdPrefill) {
    const { data: qPrefill } = await supabase
      ...
      .eq("id", quotationIdPrefill)
      .maybeSingle();
    prefillFromQuotation = (qPrefill ?? null) as unknown as FinanceQuotationRow | null;
  }

  return (
    <FinanceSalesOrdersMaster
      initialRows={rows}
