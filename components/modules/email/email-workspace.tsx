...
import { FOLDER_LABELS, resolveImapFolder } from "@/lib/email/providers";
import type {
  ComposeMode,
  EmailAccountSafe,
  EmailFolderKey,
...
import { decodeHtmlEntities } from "@/lib/email/format-body";
import { threadCountForMessage } from "@/lib/email/threading";
import {
  EMAIL_FOLDERS,
  formatEmailDate,
  parseAddressList,
        ...
        {setupError && (
          <p className="max-w-md rounded-lg border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-200">
            {setupError.includes("email_accounts")
              ? "Email module database tables are missing. Run the Supabase migration for email_accounts, then refresh."
              : setupError}
          </p>
        )}
