...
import { applyProviderPreset } from "@/lib/email/providers";
import { providerMismatchMessage } from "@/lib/email/domain-provider-hints";
import type { EmailAccountRow, EmailAccountSafe, EmailProvider } from "@/lib/types/email";
...
export async function saveEmailAccount(formData: FormData): Promise<SaveEmailAccountResult> {
  try {
    ...
    if (testConnection) {
      try {
        const { testEmailAccountConnection } = await import("@/lib/email/test-connection");
