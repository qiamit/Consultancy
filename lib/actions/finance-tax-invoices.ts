...
const LIST_PATH = "/dashboard/finance/sales/tax-invoice";
...
export async function saveFinanceTaxInvoice(formData: FormData) {
  ...
  const tax_date = str(formData, "tax_date");
  ...
  const tax_status_in = str(formData, "tax_status").toLowerCase();
  const tax_status: ProformaStatus | null =
    tax_status_in === "accepted" || tax_status_in === "cancelled"
      ? tax_status_in
      : tax_status_in === "pending"
        ? "pending"
        : null;
  if (invoice_type !== "service" && invoice_type !== "supply") {
    redirect(`${LIST_PATH}?error=type`);
  }
  if (!tax_date || !valid_until_date) redirect(`${LIST_PATH}?error=dates`);
  ...
  const tax_invoice_number_in = joinProformaInvoiceNumberParts(
    str(formData, "tax_invoice_number_prefix"),
    str(formData, "tax_invoice_number_value"),
  );
  ...
  if (id) {
    if (!tax_invoice_number_in) {
      redirect(`${LIST_PATH}?error=tax_invoice_number_required`);
    }
    const { error: uErr } = await supabase
      .from("finance_tax_invoices")
      .update({
        ...header,
        tax_invoice_number: tax_invoice_number_in,
      })
      .eq("id", id);
    if (uErr?.code === "23505") {
      redirect(`${LIST_PATH}?error=tax_invoice_number_duplicate`);
    }
    if (uErr) redirect(`${LIST_PATH}?error=db`);
    const { error: dErr } = await supabase
      .from("finance_tax_invoice_lines")
      .delete()
      .eq("tax_invoice_id", id);
    if (dErr) redirect(`${LIST_PATH}?error=db`);
    const rows = built.map((b) => ({ ...b, tax_invoice_id: id }));
  ...
  const rows = built.map((b) => ({ ...b, tax_invoice_id: newId }));
  const { error: lErr } = await supabase
    .from("finance_tax_invoice_lines")
    .insert(rows);
  if (lErr) redirect(`${LIST_PATH}?error=db`);
...
export async function deleteFinanceTaxInvoice(id: string) {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
...
export async function deleteFinanceTaxInvoiceForm(formData: FormData) {
  await deleteFinanceTaxInvoice(String(formData.get("id") ?? ""));
}

export async function deleteFinanceTaxInvoices(ids: string[]) {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
...
export async function importFinanceTaxInvoices(
  ...
  for (let i = 0; i < records.length; i++) {
    const rec = records[i];
    const tax_date = String(rec.tax_date ?? "").trim();
    const valid_until_date = String(rec.valid_until_date ?? "").trim();
    if (!tax_date || !valid_until_date) {
      return {
        ok: false,
        error: `Row ${i + 2} (after header): tax_date and valid_until_date are required.`,
      };
    }
    ...
    const header = {
      ...
      invoice_type,
      tax_status:
        String(rec.tax_status ?? "").trim().toLowerCase() === "accepted" ||
        String(rec.tax_status ?? "").trim().toLowerCase() === "cancelled"
          ? (String(rec.tax_status ?? "").trim().toLowerCase() as ProformaStatus)
          : "pending",
    ...
    const num = String(rec.tax_invoice_number ?? "").trim();
    const insertHeader = {
      ...header,
    ...
    if (iErr || !ins?.id) {
      if (iErr?.code === "23505") {
        return {
          ok: false,
          error: `Row ${i + 2} (after header): duplicate tax_invoice_number or database constraint.`,
        };
      }
      return {
        ok: false,
    ...
    const newId = ins.id as string;
    const lineRows = built.map((b) => ({ ...b, tax_invoice_id: newId }));
    const { error: lErr } = await supabase
      .from("finance_tax_invoice_lines")
      .insert(lineRows);
    if (lErr) {
      await supabase.from("finance_tax_invoices").delete().eq("id", newId);
      return {
        ok: false,
...
export async function updateFinanceTaxInvoiceStatus(input: {
  id: string;
  ...
  if (!id) return { ok: false, error: "Tax invoice id missing." };
