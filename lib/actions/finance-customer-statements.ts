...
import { joinProformaInvoiceNumberParts } from "@/lib/finance-tax-invoice-number";
import { createClient } from "@/lib/supabase/server";

const LIST_PATH = "/dashboard/finance/sales/customer-statement";
...
export async function saveFinanceCustomerStatement(formData: FormData) {
  ...
  const statement_date = str(formData, "statement_date");
  ...
  const customer_statement_status_in = str(formData, "customer_statement_status").toLowerCase();
  const customer_statement_status: ProformaStatus | null =
    customer_statement_status_in === "accepted" || customer_statement_status_in === "cancelled"
      ? customer_statement_status_in
      : customer_statement_status_in === "pending"
        ? "pending"
        : null;
  if (invoice_type !== "service" && invoice_type !== "supply") {
    redirect(`${LIST_PATH}?error=type`);
  }
  if (!statement_date || !valid_until_date) redirect(`${LIST_PATH}?error=dates`);
  ...
  const customer_statement_number_in = joinProformaInvoiceNumberParts(
    str(formData, "customer_statement_number_prefix"),
    str(formData, "customer_statement_number_value"),
  );
  ...
  const header = {
    ...
    proforma_invoice_id,
    invoice_type,
    ...(customer_statement_status ? { customer_statement_status } : {}),
  ...
  if (id) {
    if (!customer_statement_number_in) {
      redirect(`${LIST_PATH}?error=customer_statement_number_required`);
    }
    const { error: uErr } = await supabase
      .from("finance_customer_statements")
      .update({
        ...header,
        customer_statement_number: customer_statement_number_in,
      })
      .eq("id", id);
    if (uErr?.code === "23505") {
      redirect(`${LIST_PATH}?error=customer_statement_number_duplicate`);
    }
    if (uErr) redirect(`${LIST_PATH}?error=db`);
    const { error: dErr } = await supabase
      .from("finance_customer_statement_lines")
      .delete()
      .eq("customer_statement_id", id);
    if (dErr) redirect(`${LIST_PATH}?error=db`);
    const rows = built.map((b) => ({ ...b, customer_statement_id: id }));
  ...
  const rows = built.map((b) => ({ ...b, customer_statement_id: newId }));
  const { error: lErr } = await supabase
    .from("finance_customer_statement_lines")
    .insert(rows);
  if (lErr) redirect(`${LIST_PATH}?error=db`);
...
export async function deleteFinanceCustomerStatement(id: string) {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
...
export async function deleteFinanceCustomerStatementForm(formData: FormData) {
  await deleteFinanceCustomerStatement(String(formData.get("id") ?? ""));
}

export async function deleteFinanceCustomerStatements(ids: string[]) {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
...
export async function importFinanceCustomerStatements(
  ...
  for (let i = 0; i < records.length; i++) {
    const rec = records[i];
    const statement_date = String(rec.statement_date ?? "").trim();
    const valid_until_date = String(rec.valid_until_date ?? "").trim();
    if (!statement_date || !valid_until_date) {
      return {
        ok: false,
        error: `Row ${i + 2} (after header): statement_date and valid_until_date are required.`,
      };
    }
    ...
    const header = {
      ...
      sales_order_id,
      invoice_type,
      customer_statement_status:
        String(rec.customer_statement_status ?? "").trim().toLowerCase() === "accepted" ||
        String(rec.customer_statement_status ?? "").trim().toLowerCase() === "cancelled"
          ? (String(rec.customer_statement_status ?? "").trim().toLowerCase() as ProformaStatus)
          : "pending",
    ...
    const num = String(rec.customer_statement_number ?? "").trim();
    const insertHeader = {
      ...header,
      customer_statement_number: num,
      created_by: user.id,
    };
    ...
    if (iErr || !ins?.id) {
      if (iErr?.code === "23505") {
        return {
          ok: false,
          error: `Row ${i + 2} (after header): duplicate customer_statement_number or database constraint.`,
        };
      }
      return {
        ok: false,
    ...
    const newId = ins.id as string;
    const lineRows = built.map((b) => ({ ...b, customer_statement_id: newId }));
    const { error: lErr } = await supabase
      .from("finance_customer_statement_lines")
      .insert(lineRows);
    if (lErr) {
      await supabase.from("finance_customer_statements").delete().eq("id", newId);
      return {
        ok: false,
...
export async function updateFinanceCustomerStatementStatus(input: {
  id: string;
  ...
  if (!id) return { ok: false, error: "customer statement id missing." };

  const { error } = await supabase
    .from("finance_customer_statements")
    .update({
      customer_statement_status: status,
      updated_at: new Date().toISOString(),
    })
    .eq("id", id);
