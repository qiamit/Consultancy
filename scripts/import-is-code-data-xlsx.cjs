/**
 * Upsert IS Code Master from Data/IS Code Data.xlsx into Postgres.
 * Matches existing rows by (is_number, revision_year).
 *
 * Local:  node scripts/import-is-code-data-xlsx.cjs
 * Railway: railway run --service Postgres-MC1Y --environment production -- node scripts/import-is-code-data-xlsx.cjs
 */
const path = require("path");
const ExcelJS = require("exceljs");
const { Client } = require("pg");

const XLSX_PATH = path.join(__dirname, "..", "Data", "IS Code Data.xlsx");
const BATCH = 150;
...
/** Excel stores IS numbers as IFERROR formulas; read the fallback literal. */
function extractIsNumber(cell) {
  const v = cell?.value;
  if (v == null) return "";
...
function qty(raw, fallback) {
  ...
  if (/^all quantity$/i.test(s)) return "All Quantities";
  return s;
}

function nullablePm(raw) {
  const s = String(raw ?? "").trim();
  if (!s || /^not identified$/i.test(s)) return null;
  return s;
}
...
async function main() {
  ...
  const skipped = { empty: 0, badYear: 0, dupFile: 0, noTitle: 0 };
  const seenFile = new Set();

  ws.eachRow({ includeEmpty: false }, (row, n) => {
    if (n === 1) return;

    const is_number = extractIsNumber(row.getCell(1));
    if (!is_number) {
      skipped.empty++;
      return;
    }
    ...
    rows.push({
      ...
      aspect_of_is,
      product_manual_number: nullablePm(cellText(row.getCell(6))),
      is_code_title: title,
  ...
  try {
    ...
    for (let i = 0; i < toUpdate.length; i += BATCH) {
      ...
      if (updated % 500 === 0 || updated === toUpdate.length) {
    ...
    console.log(
      ...
      updated,
      "skipped",
      skipped.empty + skipped.badYear + skipped.dupFile + skipped.noTitle,
      "total is_codes",
      finalCount.rows[0].n,
    );
  } catch (e) {
