...
import { polishProcessDescriptionPoint } from "@backend/actions/process-description-assistant";
...
function QePolishButton({
  ...
  return (
    <button
      ...
      title="QE Assistant — polish and elaborate this process description point"
      aria-label="QE Assistant — polish and elaborate this process description point"
...
export function ProcessDescriptionTableEditor({
  rows,
  onChange,
}: {
  rows: string[];
  ...
  function handlePolish(index: number) {
    ...
    if (!draft) {
      window.alert("Enter process description text before using QE Assistant.");
      return;
    }

    setPolishingIndex(index);
    startPolish(async () => {
      const result = await polishProcessDescriptionPoint(draft);
                  ...
                  <div className="flex items-start gap-1.5">
                    <div className="min-w-0 flex-1">
                      <AutoResizeTextarea
                        ...
                        placeholder="Enter process description point…"
