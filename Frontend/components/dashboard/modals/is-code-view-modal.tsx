...
function ISFeeRow({ label, value }: { label: string; value: number | null }) {
  return (
    <div className="flex items-center justify-between rounded-none bg-zinc-50 px-3 py-2 dark:bg-zinc-800/50">
...
function ISSlabRow({ label, quantity, rate }: { label: string; quantity: string | null; rate: number | null }) {
  return (
    <div className="flex items-center justify-between gap-2 rounded-none bg-zinc-50 px-3 py-2 dark:bg-zinc-800/50">
...
export function IsCodeViewModal({
  ...
  return (
    <div className={`fixed inset-0 ${overlayZIndexClass} flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm`}>
      <div className="relative flex max-h-[88vh] w-full max-w-4xl flex-col rounded-none bg-white shadow-2xl dark:bg-zinc-900">
        <div className="flex items-center gap-3 rounded-none bg-gradient-to-r from-indigo-600 to-violet-600 px-5 py-4">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-none bg-white/20 text-sm font-bold text-white">IS</div>
          ...
          <button onClick={onClose} className="shrink-0 rounded-none p-1.5 text-white/70 hover:bg-white/10 hover:text-white">
        ...
        <div className="flex-1 space-y-4 overflow-y-auto px-5 py-4">
          {loading ? (
            <div className="flex justify-center py-8">
              <div className="h-6 w-6 animate-spin rounded-none border-2 border-indigo-500 border-t-transparent" />
            </div>
          ) : !isCode ? (
            <p className="py-8 text-center text-sm text-zinc-400">No IS code data found.</p>
          ) : (
            <>
              <div className="rounded-none bg-indigo-50 px-4 py-3 dark:bg-indigo-950/30">
              ...
              <div>
                ...
                <div className="grid grid-cols-4 divide-x divide-zinc-200 rounded-none border border-zinc-200 py-3 dark:divide-zinc-700 dark:border-zinc-700">
              ...
              <div>
                <p className="mb-2 text-xs font-bold uppercase tracking-wider text-zinc-400">
                  IS Code Files
                  {files.length > 0 && (
                    <span className="ml-2 rounded-none bg-indigo-100 px-2 py-0.5 text-[10px] font-semibold text-indigo-700 dark:bg-indigo-950/40 dark:text-indigo-300">
                      {files.length}
                    </span>
                  )}
                </p>
                {files.length === 0 ? (
                  <p className="rounded-none border border-dashed border-zinc-200 px-4 py-3 text-center text-xs text-zinc-400 dark:border-zinc-700">
