...
export function SelfEvaluationFormModal({
  ...
  useEffect(() => {
    ...
    void loadCompanyPrintContext().then(({ printSettings: fromDb, assetUrls }) => {
      ...
      const defaults = defaultSelfEvaluationFormPrintSettings();
  ...
  // Keep Self Evaluation margin defaults in sync (matches Top Management / Plant & Machinery).
  useEffect(() => {
    const defaults = defaultSelfEvaluationFormPrintSettings();
    setPrintSettings((prev) => ({
      ...prev,
  ...
  const refreshPreview = useCallback(() => {
    ...
    const html = buildSelfEvaluationFormHtml(previewData, printSettings, printAssets);
    doc.open();
    doc.write(html);
    doc.close();
  ...
  function handleDownloadWord() {
    void downloadSelfEvaluationFormWord(previewData, printSettings, printAssets).catch(() =>
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      await downloadPrintHtmlAsPdf({
        html,
        filename: `Self_Evaluation_Form_${safePdfFilenamePart(letterData.companyName)}.pdf`,
