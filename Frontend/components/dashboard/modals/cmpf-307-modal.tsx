...
export function Cmpf307Modal({
  ...
  const [printAssets, setPrintAssets] = useState<Cmpf307PrintAssets>({});
  ...
  useEffect(() => {
    ...
    void loadCompanyPrintContext().then(({ printSettings: fromDb, assetUrls }) => {
      ...
      const defaults = defaultCmpf307PrintSettings();
  ...
  // Keep CMPF 307 margin defaults in sync (matches Top Management / Plant & Machinery).
  useEffect(() => {
    const defaults = defaultCmpf307PrintSettings();
    setPrintSettings((prev) => ({
      ...prev,
  ...
  const refreshPreview = useCallback(() => {
    ...
    const html = buildCmpf307Html(previewData, printSettings, printAssets);
    doc.open();
    doc.write(html);
    doc.close();
  ...
  function handleDownloadWord() {
    void downloadCmpf307Word(previewData, printSettings, printAssets).catch(() =>
  ...
  async function handleDownloadPdf() {
    ...
    try {
      ...
      await downloadPrintHtmlAsPdf({
        html,
        filename: `CMPF_307_${safePdfFilenamePart(letterData.companyName)}.pdf`,
