...
import {
  ...
  type UsitAnnexFieldKey,
} from "@backend/modules/bis/updated-scheme-of-inspection";

export type { UsitAnnexFieldKey };

export const USIT_ANNEX_FIELDS = USIT_ANNEX_SECTIONS;
...
const REFINE_SYSTEM = `You refine Annex C wording for a BIS Updated Scheme of Inspection & Testing document.
Return ONLY the refined paragraph text. No markdown, no labels, no quotes.`;

export function UpdatedSitAnnexTableEditor({
  document,
  onChange,
}: {
  ...
  const extraRows = document.annex_extra_rows ?? [];
  const [refiningId, setRefiningId] = useState<string | null>(null);

  function addExtraRow() {
    onChange({ annex_extra_rows: [...extraRows, emptySitAnnexExtraRow()] });
  }

  function updateExtraRow(index: number, patch: Partial<(typeof extraRows)[number]>) {
    onChange({
      annex_extra_rows: extraRows.map((row, i) =>
  ...
  function removeExtraRow(index: number) {
    onChange({ annex_extra_rows: extraRows.filter((_, i) => i !== index) });
  }

  async function refineText(id: string, header: string, text: string, apply: (next: string) => void) {
    const trimmed = text.trim();
    if (!trimmed) {
      window.alert(`Enter ${header || "section"} text before AI refining.`);
      return;
    }
    ...
    try {
      const res = await sendAiMessage(
        [
          {
            role: "user",
            content: `Annex section: ${header || "Custom"}
Instruction: Use formal, professional BIS Product Manual / Annex C wording.
    ...
    <div className="mb-3 space-y-2">
      <div className="overflow-x-auto rounded-lg border border-zinc-700 bg-zinc-900">
        <table className="w-full min-w-[640px] border-collapse text-[11px]">
          ...
          <tbody>
            {USIT_ANNEX_FIELDS.map(({ key, header }) => (
