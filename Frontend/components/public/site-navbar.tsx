...
function MobileAccordion({
  ...
  return (
    <div className="site-mobile-nav-section">
      <button
        type="button"
        className="flex w-full cursor-pointer items-center justify-between px-3 py-2.5 text-sm font-semibold text-gray-700 rounded-lg hover:bg-blue-50 touch-manipulation"
        aria-expanded={open}
        onClick={() => setOpen((v) => !v)}
      >
        {title}
      ...
      {open ? <div className="pl-3 mt-1 space-y-0.5">{children}</div> : null}
    </div>
  );
}
...
export function SiteNavbar() {
  ...
  // Safari-safe scroll lock (class on html — avoids :has(#details[open]) quirks)
  useEffect(() => {
    const root = document.documentElement;
    if (mobileOpen) root.classList.add("mobile-nav-open");
      ...
      ? createPortal(
          <>
            <button
              ...
              className="fixed inset-0 z-[60] bg-black/25 lg:hidden touch-manipulation"
            ...
            <div
              ...
              className="fixed left-0 right-0 top-[65px] z-[70] max-h-[min(80vh,calc(100dvh-65px))] overflow-y-auto overscroll-contain border-t border-gray-100 bg-white shadow-lg dark:border-zinc-800 dark:bg-zinc-900 lg:hidden"
            >
              <div className="px-4 py-3 space-y-0.5">
                <Link href="/" className={mobileLink} onClick={closeMobileNav}>
                  Home
                </Link>
                <Link href="/about" className={mobileLink} onClick={closeMobileNav}>
                  About
                </Link>
                ...
                <MobileAccordion title="Mandatory Products">
                  <Link
                    href={MANDATORY_INTERNAL.href}
                    className="block px-3 py-2 text-sm font-semibold text-sky-600 dark:text-sky-400 rounded-lg hover:bg-blue-50"
                    onClick={closeMobileNav}
                  >
                ...
                <Link href="/updates" className={mobileLink} onClick={closeMobileNav}>
                  Updates
                </Link>
                <Link href="/testimonials" className={mobileLink} onClick={closeMobileNav}>
                  Testimonials
                </Link>
                <Link href="/contact" className={mobileLink} onClick={closeMobileNav}>
                  Contact
                </Link>
                ...
                <div className="pt-3 pb-1 border-t border-gray-100 mt-2 flex gap-2">
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-[#25D366] text-white font-bold rounded-xl text-sm touch-manipulation"
                  >
                  ...
                  <button
                    ...
                    className="flex-1 flex items-center justify-center py-2.5 bg-sky-600 text-white font-bold rounded-xl hover:bg-sky-500 transition-colors text-sm touch-manipulation"
                  >
                    Client Portal
                  </button>
                </div>
  ...
  return (
    <header
      className={`fixed top-0 left-0 right-0 z-50 w-full max-w-[100vw] transition-all duration-300 border-b border-zinc-200/80 dark:border-zinc-800 ${
        mobileOpen ? "overflow-visible" : "overflow-x-clip"
      } ${
        scrolled
    ...
    >
      ...
      <nav className="w-full px-3 sm:px-6 lg:px-8 flex items-center h-[62px] gap-2 sm:gap-4">
        {/* Mobile menu toggle — React state + portal (Safari-safe; avoids overflow clip of fixed panels) */}
        <button
          ref={menuButtonRef}
          type="button"
          id="site-mobile-nav"
          className="site-mobile-nav lg:hidden flex-shrink-0 relative z-[70] flex h-11 w-11 items-center justify-center rounded-lg bg-white/95 text-gray-700 shadow-sm ring-1 ring-zinc-200/80 hover:bg-gray-100 active:bg-gray-200 transition-colors touch-manipulation cursor-pointer select-none dark:bg-zinc-900/95 dark:text-zinc-100 dark:ring-zinc-700"
          ...
          <div className="flex w-full items-stretch gap-0.5 xl:gap-1 rounded-xl border border-zinc-200 dark:border-zinc-700 bg-zinc-100/90 dark:bg-zinc-800/80 p-1">
            ...
            <div className="relative group flex flex-1 min-w-0">
              ...
              <div className={`${dropdownPanel} min-w-[300px] xl:min-w-[320px]`}>
                ...
                {SERVICES.map((s) => (
                  <Link key={s.href} href={s.href} className={dropdownLink}>
