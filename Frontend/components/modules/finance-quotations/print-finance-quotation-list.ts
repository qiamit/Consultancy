...
function listTableHtml(rows: FinanceQuotationRow[]): string {
  const head =
    "<thead><tr><th>#</th><th>Quotation #</th><th>Date</th><th>Expires</th><th>Client</th><th>Type</th><th class=\"num\">Total</th></tr></thead>";
  const body = rows
    .map(
      (r, i) =>
...
function openPrintWindow(html: string): void {
  const existing = document.getElementById("finance-quotations-print-root");
  existing?.remove();

  const root = document.createElement("div");
  root.id = "finance-quotations-print-root";
  root.setAttribute(
    "style",
    [
      "position:fixed",
...
/** Print selected / filtered quotations — tabular list. */
