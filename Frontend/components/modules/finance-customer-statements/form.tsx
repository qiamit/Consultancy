...
import { FINANCE_LINES_WITH_GST_COLUMNS } from "@/components/modules/finance/finance-lines-table-columns";
...
import { DROPDOWN_KEY_FINANCE_CUSTOMER_STATEMENT_CLIENT } from "@backend/shared/dropdown-keys";
...
import {
  deleteFinanceCustomerStatementForm,
  saveFinanceCustomerStatement,
...
import type { CustomerStatementFormState, CustomerStatementLineForm } from "./constants";
...
function linePreview(L: CustomerStatementLineForm) {
...
export function FinanceCustomerStatementForm({
  visible,
  overlay,
  formValues,
  isNewParam,
  ...
  visible: boolean;
  overlay?: boolean;
  formValues: CustomerStatementFormState;
  isNewParam: boolean;
  idParam: string | null;
  ...
  onClose: () => void;
  onAddNew: () => void;
  onUpdateField: (key: keyof CustomerStatementFormState, value: string) => void; // invoice_type: 'service' | 'supply'
  onUpdateLine: (index: number, patch: Partial<CustomerStatementLineForm>) => void;
  onAddLine: () => void;
  onRemoveLine: (index: number) => void;
  onStatementDateChange: (iso: string) => void;
  /** Current quotation URL (path + ?new=1 or ?id=) for “add client” return navigation. */
  customerStatementReturnUrl: string;
  ...
  const taxDisplayNumber =
    `${formValues.customer_statement_number_prefix}${formValues.customer_statement_number_value}`.trim();
  ...
  body,.finance-customer-statement-pdf-mount{font-family:Arial,sans-serif;color:#111;margin:0}
  ...
  <div class="pad grid">
    <div><div class="label">Customer Statement no.</div><div>${esc(taxDisplayNumber || "-")}</div></div>
    <div class="right"><div class="title">PROFORMA INVOICE</div><div class="muted">Date: ${esc(formatDisplayDate(formValues.statement_date, "-"))} | Valid until: ${esc(formatDisplayDate(formValues.valid_until_date, "-"))}</div></div>
  </div>
  <div class="section">
  ...
  const buildTaxInvoiceDocumentHtml = () => {
    ...
    return `<!doctype html>
<html><head><meta charset="utf-8" /><title>Customer Statement ${esc(taxDisplayNumber || "")}</title>
...
<div class="doc">${docInner}</div></body></html>`;
  ...
  const printTaxInvoice = async () => {
    ...
    openPrintPreview({
      buildDoc: (s, c) => buildPrintDocument({
        title: `Customer Statement ${taxDisplayNumber || ""}`,
  ...
  const createTaxInvoicePdfBlob = async (): Promise<Blob> => {
    ...
    mount.className = "finance-customer-statement-pdf-mount";
    mount.style.cssText =
    ...
    if (!target) {
      mount.remove();
      throw new Error("Customer Statement PDF mount missing .doc root");
    }
    ...
    try {
      ...
      const blob = (await html2pdf()
        ...
        .set({
          margin: [6, 6, 6, 6],
          filename: `${taxDisplayNumber || "customer-statement"}.pdf`,
  ...
  const downloadTaxInvoicePdf = async () => {
    ...
    a.href = url;
    a.download = `${taxDisplayNumber || "customer-statement"}.pdf`;
    a.click();
    URL.revokeObjectURL(url);
  };
  const shareTaxInvoice = async () => {
    const shareText = `Customer Statement ${taxDisplayNumber || formValues.id}`;
    try {
      const blob = await createTaxInvoicePdfBlob();
      const file = new File([blob], `${taxDisplayNumber || "customer-statement"}.pdf`, {
        type: "application/pdf",
      });
      if (navigator.share && (navigator as Navigator & { canShare?: (data: ShareData) => boolean }).canShare?.({ files: [file] })) {
        await navigator.share({ title: "Customer Statement", text: shareText, files: [file] });
        return;
      }
  ...
  return (
    <div className={rootClass}>
        <FinanceFormOverlayHeader
          titleId="finance-customer-statement-form-title"
          title={
            isNewParam
          ...
          addNewLabel="Add New Customer Statement"
          onAddNew={onAddNew}
        />

        <form action={saveFinanceCustomerStatement} className="space-y-5">
          ...
          <div className={FINANCE_FORM_HEADER_GRID_CLASS}>
              ...
              <div className={FINANCE_FORM_HEADER_ROW_CLASS}>
                <div className={FINANCE_FORM_NUMBER_FIELD_CLASS}>
                  <span className={CLIENT_FIELD_LABEL_BLOCK_CLASS}>
                    Customer Statement number
                  </span>
                  <div
                    className={quotationNumberSplitShell}
                    role="group"
                    aria-label="Customer Statement number"
                  >
                    <input
                      id="finance-quotation-number-prefix"
                      name="customer_statement_number_prefix"
                      type="text"
                      aria-label="Prefix"
                      value={formValues.customer_statement_number_prefix}
                      onChange={(e) =>
                        onUpdateField(
                      ...
                      placeholder={formValues.id ? undefined : "CS-2026-"}
                      autoComplete="off"
                      spellCheck={false}
                    ...
                    <input
                      ...
                      name="customer_statement_number_value"
                      type="text"
                      aria-label="Number"
                      value={formValues.customer_statement_number_value}
                      onChange={(e) =>
                        onUpdateField(
            ...
            {/* Line 2: client */}
              <div className={FINANCE_FORM_CLIENT_FIELD_CLASS}>
                <ClientDropdownField
                  optionKey={DROPDOWN_KEY_FINANCE_CUSTOMER_STATEMENT_CLIENT}
                  name="client_id"
                  label="Client"
                  dialogTitle="Clients"
          ...
          <div className="space-y-2">
            <FinanceLinesSectionHeader
              availableColumns={FINANCE_LINES_WITH_GST_COLUMNS}
          ...
          <div className="flex flex-wrap items-center justify-end gap-3 border-t border-zinc-200 pt-4 dark:border-zinc-800">
            ...
            {formValues.id ? (
              <button
                type="submit"
                formAction={deleteFinanceCustomerStatementForm}
