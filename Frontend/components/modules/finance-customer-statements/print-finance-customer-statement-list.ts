import { formatDisplayDate, formatPrintTimestamp } from "@backend/shared/format-date";
...
function listTableHtml(rows: FinanceCustomerStatementRow[]): string {
  const head =
    "<thead><tr><th>#</th><th>Customer Statement #</th><th>Date</th><th>Valid until</th><th>Linked SO</th><th>Client</th><th>Type</th><th class=\"num\">Total</th></tr></thead>";
  const body = rows
    .map(
      (r, i) =>
        `<tr><td>${i + 1}</td><td class="mono">${esc(r.customer_statement_number)}</td><td>${esc(formatDisplayDate(r.statement_date))}</td><td>${esc(formatDisplayDate(r.valid_until_date))}</td><td class="mono">${esc(r.finance_sales_orders?.sales_order_number ?? "—")}</td><td>${esc(clientLabel(r))}</td><td>${esc(r.invoice_type)}</td><td class="num">${esc(
...
function openPrintWindow(html: string): void {
  const existing = document.getElementById("finance-customer-statements-print-root");
  existing?.remove();

  const root = document.createElement("div");
  root.id = "finance-customer-statements-print-root";
  root.setAttribute(
    "style",
    [
      "position:fixed",
...
/** Print selected / filtered customer statements — tabular list. */
export function printFinanceCustomerStatementsList(rows: FinanceCustomerStatementRow[]): void {
