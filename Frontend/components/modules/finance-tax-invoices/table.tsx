  ...
  return (
    <input
      ...
      title="Select all on this page"
                  aria-label="Select all tax invoices on this page"
    />
  );
}

export function FinanceTaxInvoicesTable({
  rows,
  idParam,
  onEditRow,
  onDownloadRow,
  ...
  idParam: string | null;
  onEditRow: (r: FinanceTaxInvoiceRow) => void;
  onDownloadRow: (r: FinanceTaxInvoiceRow) => void;
  onShareRow: (r: FinanceTaxInvoiceRow) => void | Promise<void>;
  onStatusChange: (
      ...
      <table className="min-w-[920px] w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-800">
        <thead className="bg-zinc-100 text-left text-xs font-semibold tracking-wide text-zinc-600 dark:bg-zinc-800/80 dark:text-zinc-400">
          <tr>
            ...
            <th className="min-w-[160px] px-3 py-2">Tax invoice details</th>
        ...
        <tbody className="divide-y divide-zinc-200 bg-white dark:divide-zinc-800 dark:bg-zinc-950/40">
          {emptyMaster ? (
            <tr>
              ...
              >
                No tax invoices yet. Use &quot;Add New Tax Invoice&quot; to open the
                form and save.
              </td>
            </tr>
          ) : noMatches ? (
            <tr>
              <td
                ...
                className="px-4 py-10 text-center text-zinc-500"
              >
                No tax invoices match your search. Try different keywords or clear
                the search box.
              </td>
            </tr>
          ) : (
            rows.map((r) => {
              ...
              const rowLabel = `Select ${r.tax_invoice_number}`;
              return (
                <tr
                  key={r.id}
                  className={
                ...
                >
                  ...
                  <td className="align-top px-2 py-2 text-right">
                    ...
                    >
                      ...
                      <Link
                        href={`${CREDIT_NOTE_LIST_PATH}?new=1&tax_invoice_id=${encodeURIComponent(r.id)}`}
                      ...
                      <span className="text-zinc-400 dark:text-zinc-600">|</span>
                      <Link
                        href={`${CUSTOMER_STATEMENT_LIST_PATH}?new=1&tax_invoice_id=${encodeURIComponent(r.id)}`}
