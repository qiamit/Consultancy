...
function clientLabel(r: FinanceTaxInvoiceRow): string {
  const c = r.clients;
  if (!c) return "—";
...
function listTableHtml(rows: FinanceTaxInvoiceRow[]): string {
  const head =
    "<thead><tr><th>#</th><th>Tax Invoice #</th><th>Date</th><th>Valid until</th><th>Linked SO</th><th>Client</th><th>Type</th><th class=\"num\">Total</th></tr></thead>";
  const body = rows
    .map(
      (r, i) =>
        `<tr><td>${i + 1}</td><td class="mono">${esc(r.tax_invoice_number)}</td><td>${esc(formatDisplayDate(r.tax_date))}</td><td>${esc(formatDisplayDate(r.valid_until_date))}</td><td class="mono">${esc(r.finance_sales_orders?.sales_order_number ?? "—")}</td><td>${esc(clientLabel(r))}</td><td>${esc(r.invoice_type)}</td><td class="num">${esc(
...
function openPrintWindow(html: string): void {
  const existing = document.getElementById("finance-tax-invoices-print-root");
  existing?.remove();

  const root = document.createElement("div");
  root.id = "finance-tax-invoices-print-root";
  root.setAttribute(
    "style",
    [
      "position:fixed",
...
/** Print selected / filtered tax invoices — tabular list. */
export function printFinanceTaxInvoicesList(rows: FinanceTaxInvoiceRow[]): void {
