...
import {
  deleteFinanceTaxInvoice,
  deleteFinanceTaxInvoices,
  importFinanceTaxInvoices,
  updateFinanceTaxInvoiceStatus,
} from "@backend/actions/finance-tax-invoices";
import {
  buildFinanceTaxInvoicesExportCsv,
  parseFinanceTaxInvoicesImportCsv,
} from "@backend/modules/finance/finance-tax-invoices-csv";
...
import {
  emptyForm,
  emptyLine,
  TAX_INVOICE_LIST_PATH,
  rowToForm,
  splitTaxInvoiceNumberForForm,
  type TaxInvoiceFormState,
  type TaxInvoiceLineForm,
} from "./constants";
import { FinanceTaxInvoiceForm } from "./form";
import { FinanceTaxInvoicesHeaderBar } from "./header-bar";
import { printFinanceTaxInvoicesList } from "./print-finance-tax-invoice-list";
import { filterTaxInvoicesBySearch, PAGE_SIZE_OPTIONS } from "./search-utils";
import { FinanceTaxInvoicesTable } from "./table";
...
function salesOrderToTaxDraft(
  ...
  return {
    ...
    sales_order_id: q.id,
    proforma_invoice_id: "",
    tax_invoice_number_prefix: nextNumber.tax_invoice_number_prefix,
    tax_invoice_number_value: nextNumber.tax_invoice_number_value,
    tax_date: q.order_date,
...
function getNextTaxInvoiceNumberParts(
  ...
  if (rows.length === 0) {
    return {
      tax_invoice_number_prefix: preferredPrefixOverride ?? "",
      tax_invoice_number_value: "00001",
    };
  }
  ...
  const latestParts = splitTaxInvoiceNumberForForm(
    sortedByCreatedAt[0]?.tax_invoice_number ?? "",
  );
  const preferredPrefix =
    preferredPrefixOverride ?? latestParts.tax_invoice_number_prefix;

  let maxValue = 0;
  let width = 5;

  for (const row of rows) {
    const parts = splitTaxInvoiceNumberForForm(row.tax_invoice_number ?? "");
    if (parts.tax_invoice_number_prefix !== preferredPrefix) continue;
    if (!/^\d+$/.test(parts.tax_invoice_number_value)) continue;

    const parsed = Number(parts.tax_invoice_number_value);
    if (!Number.isFinite(parsed)) continue;

    maxValue = Math.max(maxValue, parsed);
    width = Math.max(width, parts.tax_invoice_number_value.length);
  }

  const next = String(maxValue + 1).padStart(width, "0");
  return {
    tax_invoice_number_prefix: preferredPrefix,
    tax_invoice_number_value: next,
  };
}

export function FinanceTaxInvoicesMaster({
  initialRows,
  fetchError,
  queryError,
  clientRows,
  ...
  const taxInvoiceReturnUrl = useMemo(() => {
    ...
    const s = q.toString();
    return s ? `${TAX_INVOICE_LIST_PATH}?${s}` : TAX_INVOICE_LIST_PATH;
  }, [isNewParam, idParam]);
  ...
  useEffect(() => {
    ...
    q.delete("client_id");
    const qs = q.toString();
    router.replace(`${TAX_INVOICE_LIST_PATH}${qs ? `?${qs}` : ""}`, {
      scroll: false,
    });
  ...
  const selectRow = useCallback(
    (r: FinanceTaxInvoiceRow) => {
      router.replace(`${TAX_INVOICE_LIST_PATH}?id=${encodeURIComponent(r.id)}`, {
        scroll: false,
      });
    },
    [router],
  ...
  const downloadRow = useCallback((r: FinanceTaxInvoiceRow) => {
    ...
    a.href = url;
    a.download = `${(r.tax_invoice_number || "tax-invoice").trim()}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }, []);

  const shareRow = useCallback(
    async (r: FinanceTaxInvoiceRow) => {
      const url = `${window.location.origin}${TAX_INVOICE_LIST_PATH}?id=${encodeURIComponent(r.id)}`;
      const text = `Tax invoice ${r.tax_invoice_number || r.id}`;
      try {
        if (navigator.share) {
          await navigator.share({ title: "Tax invoice", text, url });
          return;
        }
  ...
  const updateRowStatus = useCallback(
    async (
      ...
      setRows((prev) =>
        prev.map((x) => (x.id === r.id ? { ...x, tax_status: status } : x)),
      );
      const result = await updateFinanceTaxInvoiceStatus({ id: r.id, status });
      if (!result.ok) {
        setRows((prev) =>
          prev.map((x) =>
            x.id === r.id
              ? { ...x, tax_status: r.tax_status ?? "pending" }
              : x,
          ),
        );
  ...
  const addNew = useCallback(() => {
    router.replace(`${TAX_INVOICE_LIST_PATH}?new=1`, { scroll: false });
  }, [router]);

  const closeForm = useCallback(() => {
    router.replace(TAX_INVOICE_LIST_PATH, { scroll: false });
  }, [router]);
  ...
  function handleExport() {
    const csv = buildFinanceTaxInvoicesExportCsv(filtered);
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "finance-tax-invoices-export.csv";
    a.click();
    URL.revokeObjectURL(url);
  }
    ...
    if (toPrint.length === 0) {
      window.alert(
        selectedIds.size > 0
          ...
          : "No rows to print. Adjust your search or add tax invoices.",
      );
      return;
    }
    printFinanceTaxInvoicesList(toPrint);
  }

  async function handleImportFile(file: File) {
    const text = await file.text();
    const parsed = parseFinanceTaxInvoicesImportCsv(text);
    if (!parsed.ok) {
      window.alert(parsed.error);
      return;
    }
    const result = await importFinanceTaxInvoices(parsed.rows);
    if (!result.ok) {
      window.alert(result.error);
      return;
    }
    window.alert(`Imported ${result.inserted} tax invoice(s).`);
    router.refresh();
  }
  ...
  function handleDelete() {
    ...
    if (bulkIds.length > 0) {
      ...
      if (
        !window.confirm(
          n === 1
            ? "Delete this tax invoice permanently? This cannot be undone."
            : `Delete ${n} tax invoices permanently? This cannot be undone.`,
        )
      ) {
        return;
      }
      void deleteFinanceTaxInvoices(bulkIds);
      return;
    }
    ...
    void deleteFinanceTaxInvoice(idParam);
  }
  ...
  const errMsg =
    queryError === "dates"
      ? "Tax invoice date and due-date date are required."
      : queryError === "type"
        ? "Invalid invoice type."
        : queryError === "lines"
          ? "Add at least one line with quantity, rate, or product."
          : queryError === "tax_invoice_number_required"
            ? "Tax invoice number is required."
            : queryError === "tax_invoice_number_duplicate"
    ...
    <div className="w-full max-w-none space-y-0">
      ...
      {formVisible ? (
        <FinanceFormModalShell
          ariaLabelledBy="finance-tax-invoice-form-title"
          onClose={closeForm}
        >
            <FinanceTaxInvoiceForm
              visible
              overlay
              formValues={form}
              ...
              taxInvoiceReturnUrl={taxInvoiceReturnUrl}
