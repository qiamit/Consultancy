...
import { DROPDOWN_KEY_FINANCE_PROFORMA_INVOICE_CLIENT } from "@backend/shared/dropdown-keys";
...
import {
  deleteFinanceProformaInvoiceForm,
  saveFinanceProformaInvoice,
...
function linePreview(L: ProformaInvoiceLineForm) {
...
export function FinanceProformaInvoiceForm({
  ...
  onAddLine,
  onRemoveLine,
  onProformaDateChange,
  proformaInvoiceReturnUrl,
  sealSignImageUrl,
  ...
  visible: boolean;
  overlay?: boolean;
  formValues: ProformaInvoiceFormState;
  isNewParam: boolean;
  idParam: string | null;
  ...
  onClose: () => void;
  onAddNew: () => void;
  onUpdateField: (key: keyof ProformaInvoiceFormState, value: string) => void; // invoice_type: 'service' | 'supply'
  onUpdateLine: (index: number, patch: Partial<ProformaInvoiceLineForm>) => void;
  onAddLine: () => void;
  onRemoveLine: (index: number) => void;
  onProformaDateChange: (iso: string) => void;
  /** Current quotation URL (path + ?new=1 or ?id=) for “add client” return navigation. */
  proformaInvoiceReturnUrl: string;
  ...
  useEffect(() => {
    ...
    try {
      ...
      const match = entries.find((e) =>
        e.name.toLowerCase().includes("proforma"),
      );
      if (match?.prefix && !formValues.proforma_invoice_number_prefix) {
        onUpdateField("proforma_invoice_number_prefix", match.prefix);
      }
    } catch { /* ignore */ }
  ...
  const proformaDisplayNumber =
    `${formValues.proforma_invoice_number_prefix}${formValues.proforma_invoice_number_value}`.trim();
  ...
  const buildProformaInvoiceDocumentParts = () => {
  ...
  body,.finance-proforma-invoice-pdf-mount{font-family:Arial,sans-serif;color:#111;margin:0}
  ...
  const buildProformaInvoiceDocumentHtml = () => {
    const { styles, docInner } = buildProformaInvoiceDocumentParts();
    return `<!doctype html>
<html><head><meta charset="utf-8" /><title>Proforma invoice ${esc(proformaDisplayNumber || "")}</title>
<style>${styles}</style></head><body>
<div class="doc">${docInner}</div></body></html>`;
  };
  const printProformaInvoice = async () => {
    ...
    openPrintPreview({
      buildDoc: (s, c) => buildPrintDocument({
        title: `Proforma Invoice ${proformaDisplayNumber || ""}`,
        bodyHtml: buildProformaInvoiceDocumentParts().docInner,
        settings: s,
        company: c,
        extraStyles: "",
      }),
  ...
  const createProformaInvoicePdfBlob = async (): Promise<Blob> => {
    const html2pdf = (await import("html2pdf.js")).default;
    const { styles, docInner } = buildProformaInvoiceDocumentParts();
    const mount = document.createElement("div");
    mount.className = "finance-proforma-invoice-pdf-mount";
    mount.style.cssText =
    ...
    if (!target) {
      mount.remove();
      throw new Error("Proforma invoice PDF mount missing .doc root");
    }
    ...
    try {
      ...
      const blob = (await html2pdf()
        ...
        .set({
          margin: [6, 6, 6, 6],
          filename: `${proformaDisplayNumber || "proforma-invoice"}.pdf`,
  ...
  const downloadProformaInvoicePdf = async () => {
    const blob = await createProformaInvoicePdfBlob();
    const url = URL.createObjectURL(blob);
    const a = document.createElement("a");
    a.href = url;
    a.download = `${proformaDisplayNumber || "proforma-invoice"}.pdf`;
    a.click();
    URL.revokeObjectURL(url);
  };
  const shareProformaInvoice = async () => {
    const shareText = `Proforma invoice ${proformaDisplayNumber || formValues.id}`;
    try {
      const blob = await createProformaInvoicePdfBlob();
      const file = new File([blob], `${proformaDisplayNumber || "proforma-invoice"}.pdf`, {
        type: "application/pdf",
      });
      if (navigator.share && (navigator as Navigator & { canShare?: (data: ShareData) => boolean }).canShare?.({ files: [file] })) {
        await navigator.share({ title: "Proforma invoice", text: shareText, files: [file] });
        return;
      }
  ...
  return (
    <div className={rootClass}>
        <FinanceFormOverlayHeader
          titleId="finance-proforma-invoice-form-title"
          title={
            isNewParam
          ...
          addNewLabel="Add New Proforma"
          onAddNew={onAddNew}
        />

        <form action={saveFinanceProformaInvoice} className="space-y-5">
          ...
          <div className={FINANCE_FORM_HEADER_GRID_CLASS}>
              ...
              <div className={FINANCE_FORM_HEADER_ROW_CLASS}>
                <div className={FINANCE_FORM_NUMBER_FIELD_CLASS}>
                  ...
                  <div
                    ...
                    role="group"
                    aria-label="Proforma number"
                  >
                    <input
                      id="finance-quotation-number-prefix"
                      name="proforma_invoice_number_prefix"
                      type="text"
                      aria-label="Prefix"
                      value={formValues.proforma_invoice_number_prefix}
                      onChange={(e) =>
                        onUpdateField(
                      ...
                      placeholder={formValues.id ? undefined : "PF-2026-"}
                      autoComplete="off"
                      spellCheck={false}
                    ...
                    <input
                      ...
                      name="proforma_invoice_number_value"
                      type="text"
                      aria-label="Number"
                      value={formValues.proforma_invoice_number_value}
                      onChange={(e) =>
                        onUpdateField(
                ...
                <div className={FINANCE_FORM_DATE_FIELD_CLASS}>
                  ...
                  <input
                    ...
                    name="proforma_date"
                    required
                    value={formValues.proforma_date}
                    onChange={(e) => {
                      const v = e.target.value;
                      onProformaDateChange(v);
                    }}
            ...
            {/* Line 2: client */}
              <div className={FINANCE_FORM_CLIENT_FIELD_CLASS}>
                <ClientDropdownField
                  optionKey={DROPDOWN_KEY_FINANCE_PROFORMA_INVOICE_CLIENT}
                  name="client_id"
                  label="Client"
                  dialogTitle="Clients"
          ...
          <div className="flex flex-wrap items-center justify-end gap-3 border-t border-zinc-200 pt-4 dark:border-zinc-800">
            {formValues.id ? (
              <>
                <button
                  type="button"
                  onClick={downloadProformaInvoicePdf}
                ...
                <button
                  type="button"
                  onClick={() => void printProformaInvoice()}
            ...
            {formValues.id ? (
              <button
                type="submit"
                formAction={deleteFinanceProformaInvoiceForm}
