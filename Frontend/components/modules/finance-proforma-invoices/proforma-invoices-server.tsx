import { FinanceProformaInvoicesMaster } from "./master";
...
export async function FinanceProformaInvoicesServer({
  searchParams,
}: {
  ...
  const [
    ...
    notesTemplates,
      ...
      supabase
        ...
        .select(
          `id, proforma_invoice_number, proforma_status, proforma_date, valid_until_date, client_id, sales_order_id, invoice_type,
          ...
          finance_proforma_invoice_lines(
            id, proforma_invoice_id, sort_order, product_master_item_id, item_description,
  ...
  const rows = (qRaw ?? []) as unknown as FinanceProformaInvoiceRow[];
  for (const r of rows) {
