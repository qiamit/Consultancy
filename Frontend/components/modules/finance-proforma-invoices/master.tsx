...
import type { FinanceProformaInvoiceRow } from "@backend/shared/types/finance-proforma-invoice";
import {
  deleteFinanceProformaInvoice,
  deleteFinanceProformaInvoices,
  importFinanceProformaInvoices,
  updateFinanceProformaInvoiceStatus,
} from "@backend/actions/finance-proforma-invoices";
import {
  buildFinanceProformaInvoicesExportCsv,
  parseFinanceProformaInvoicesImportCsv,
} from "@backend/modules/finance/finance-proforma-invoices-csv";
...
import {
  emptyForm,
  emptyLine,
  PROFORMA_INVOICE_LIST_PATH,
  rowToForm,
  splitProformaInvoiceNumberForForm,
  type ProformaInvoiceFormState,
  type ProformaInvoiceLineForm,
} from "./constants";
import { FinanceProformaInvoiceForm } from "./form";
import { FinanceProformaInvoicesHeaderBar } from "./header-bar";
import { printFinanceProformaInvoicesList } from "./print-finance-proforma-invoice-list";
import { filterProformaInvoicesBySearch, PAGE_SIZE_OPTIONS } from "./search-utils";
import { FinanceProformaInvoicesTable } from "./table";
...
function salesOrderToProformaDraft(
  ...
  return {
    id: "",
    sales_order_id: q.id,
    proforma_invoice_number_prefix: nextNumber.proforma_invoice_number_prefix,
    proforma_invoice_number_value: nextNumber.proforma_invoice_number_value,
    proforma_date: q.order_date,
...
function getNextProformaInvoiceNumberParts(
  ...
  if (rows.length === 0) {
    return {
      proforma_invoice_number_prefix: preferredPrefixOverride ?? "",
      proforma_invoice_number_value: "00001",
    };
  }
  ...
  const latestParts = splitProformaInvoiceNumberForForm(
    sortedByCreatedAt[0]?.proforma_invoice_number ?? "",
  );
  const preferredPrefix =
    preferredPrefixOverride ?? latestParts.proforma_invoice_number_prefix;

  let maxValue = 0;
  let width = 5;

  for (const row of rows) {
    const parts = splitProformaInvoiceNumberForForm(row.proforma_invoice_number ?? "");
    if (parts.proforma_invoice_number_prefix !== preferredPrefix) continue;
    if (!/^\d+$/.test(parts.proforma_invoice_number_value)) continue;

    const parsed = Number(parts.proforma_invoice_number_value);
    if (!Number.isFinite(parsed)) continue;

    maxValue = Math.max(maxValue, parsed);
    width = Math.max(width, parts.proforma_invoice_number_value.length);
  }

  const next = String(maxValue + 1).padStart(width, "0");
  return {
    proforma_invoice_number_prefix: preferredPrefix,
    proforma_invoice_number_value: next,
  };
}

export function FinanceProformaInvoicesMaster({
  initialRows,
  fetchError,
  queryError,
  clientRows,
  ...
  initialRows: FinanceProformaInvoiceRow[];
  ...
  const [form, setForm] = useRouteBoundFormState<ProformaInvoiceFormState>(
    formVisible
      ? isNewParam
    ...
    (prev) => {
      if (isNewParam) {
        const prevPrefix = prev.proforma_invoice_number_prefix.trim();
        const nextNumber = getNextProformaInvoiceNumberParts(
          initialRows,
          prevPrefix || undefined,
        );
        if (prefillFromSalesOrder) {
          return salesOrderToProformaDraft(
  ...
  const proformaInvoiceReturnUrl = useMemo(() => {
    const q = new URLSearchParams();
    if (isNewParam) q.set("new", "1");
    else if (idParam) q.set("id", idParam);
    const s = q.toString();
    return s ? `${PROFORMA_INVOICE_LIST_PATH}?${s}` : PROFORMA_INVOICE_LIST_PATH;
  }, [isNewParam, idParam]);
  ...
  useEffect(() => {
    ...
    q.delete("client_id");
    const qs = q.toString();
    router.replace(`${PROFORMA_INVOICE_LIST_PATH}${qs ? `?${qs}` : ""}`, {
      scroll: false,
    });
  ...
  const filtered = useMemo(
    () => filterProformaInvoicesBySearch(rows, searchQuery),
    [rows, searchQuery],
  );
  ...
  const selectRow = useCallback(
    (r: FinanceProformaInvoiceRow) => {
      router.replace(`${PROFORMA_INVOICE_LIST_PATH}?id=${encodeURIComponent(r.id)}`, {
        scroll: false,
      });
    },
    [router],
  ...
  const downloadRow = useCallback((r: FinanceProformaInvoiceRow) => {
    ...
    a.href = url;
    a.download = `${(r.proforma_invoice_number || "proforma-invoice").trim()}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }, []);

  const shareRow = useCallback(
    async (r: FinanceProformaInvoiceRow) => {
      const url = `${window.location.origin}${PROFORMA_INVOICE_LIST_PATH}?id=${encodeURIComponent(r.id)}`;
      const text = `Proforma invoice ${r.proforma_invoice_number || r.id}`;
      try {
        if (navigator.share) {
          await navigator.share({ title: "Proforma invoice", text, url });
          return;
        }
        await navigator.clipboard.writeText(url);
        window.alert("Proforma invoice link copied to clipboard.");
      } catch {
  ...
  const updateRowStatus = useCallback(
    async (
      ...
      setRows((prev) =>
        prev.map((x) => (x.id === r.id ? { ...x, proforma_status: status } : x)),
      );
      const result = await updateFinanceProformaInvoiceStatus({ id: r.id, status });
      if (!result.ok) {
        setRows((prev) =>
          prev.map((x) =>
            x.id === r.id
              ? { ...x, proforma_status: r.proforma_status ?? "pending" }
              : x,
          ),
        );
  ...
  const addNew = useCallback(() => {
    router.replace(`${PROFORMA_INVOICE_LIST_PATH}?new=1`, { scroll: false });
  }, [router]);

  const closeForm = useCallback(() => {
    router.replace(PROFORMA_INVOICE_LIST_PATH, { scroll: false });
  }, [router]);

  const updateField = useCallback((key: keyof ProformaInvoiceFormState, value: string) => {
    setForm((f) => {
      if (key === "invoice_type") {
  ...
  const updateLine = useCallback((index: number, patch: Partial<ProformaInvoiceLineForm>) => {
    setForm((f) => {
  ...
  const onProformaDateChange = useCallback((iso: string) => {
    setForm((f) => ({
      ...f,
      proforma_date: iso,
  ...
  function handleExport() {
    const csv = buildFinanceProformaInvoicesExportCsv(filtered);
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "finance-proforma-invoices-export.csv";
    a.click();
    URL.revokeObjectURL(url);
  }
    ...
    if (toPrint.length === 0) {
      window.alert(
        selectedIds.size > 0
          ...
          : "No rows to print. Adjust your search or add proforma invoices.",
      );
      return;
    }
    printFinanceProformaInvoicesList(toPrint);
  }

  async function handleImportFile(file: File) {
    const text = await file.text();
    const parsed = parseFinanceProformaInvoicesImportCsv(text);
    if (!parsed.ok) {
      window.alert(parsed.error);
      return;
    }
    const result = await importFinanceProformaInvoices(parsed.rows);
    if (!result.ok) {
      window.alert(result.error);
      return;
    }
    window.alert(`Imported ${result.inserted} proforma invoice(s).`);
    router.refresh();
  }
  ...
  function handleDelete() {
    ...
    if (bulkIds.length > 0) {
      ...
      if (
        !window.confirm(
          n === 1
            ? "Delete this proforma invoice permanently? This cannot be undone."
            : `Delete ${n} proforma invoices permanently? This cannot be undone.`,
        )
      ) {
        return;
      }
      void deleteFinanceProformaInvoices(bulkIds);
      return;
    }
    ...
    void deleteFinanceProformaInvoice(idParam);
  }
  ...
  const errMsg =
    queryError === "dates"
      ? "Proforma date and valid-until date are required."
      : queryError === "type"
        ? "Invalid invoice type."
        : queryError === "lines"
          ? "Add at least one line with quantity, rate, or product."
          : queryError === "proforma_invoice_number_required"
            ? "Proforma number is required."
            : queryError === "proforma_invoice_number_duplicate"
  ...
  return (
    <div className="w-full max-w-none space-y-0">
      ...
      <div className="overflow-hidden rounded-lg border border-zinc-200 bg-white shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
        <FinanceProformaInvoicesHeaderBar
          onAddNew={addNew}
      ...
      {formVisible ? (
        <FinanceFormModalShell
          ariaLabelledBy="finance-proforma-invoice-form-title"
          onClose={closeForm}
        >
            <FinanceProformaInvoiceForm
              visible
              overlay
              formValues={form}
              ...
              onProformaDateChange={onProformaDateChange}
              proformaInvoiceReturnUrl={proformaInvoiceReturnUrl}
