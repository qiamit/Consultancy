  ...
  return (
    <input
      ...
      title="Select all on this page"
                  aria-label="Select all proforma invoices on this page"
    />
  );
}

export function FinanceProformaInvoicesTable({
  rows,
  idParam,
  onEditRow,
  onDownloadRow,
  ...
  idParam: string | null;
  onEditRow: (r: FinanceProformaInvoiceRow) => void;
  onDownloadRow: (r: FinanceProformaInvoiceRow) => void;
  onShareRow: (r: FinanceProformaInvoiceRow) => void | Promise<void>;
  onStatusChange: (
      ...
      <table className="min-w-[920px] w-full divide-y divide-zinc-200 text-sm dark:divide-zinc-800">
        <thead className="bg-zinc-100 text-left text-xs font-semibold tracking-wide text-zinc-600 dark:bg-zinc-800/80 dark:text-zinc-400">
          <tr>
            ...
            <th className="min-w-[160px] px-3 py-2">Proforma details</th>
        ...
        <tbody className="divide-y divide-zinc-200 bg-white dark:divide-zinc-800 dark:bg-zinc-950/40">
          {emptyMaster ? (
            <tr>
              ...
              >
                No proforma invoices yet. Use &quot;Add New Proforma&quot; to open the
                form and save.
              </td>
            </tr>
          ) : noMatches ? (
            <tr>
              <td
                ...
                className="px-4 py-10 text-center text-zinc-500"
              >
                No proforma invoices match your search. Try different keywords or clear
                the search box.
              </td>
            </tr>
          ) : (
            rows.map((r) => {
              ...
              const rowLabel = `Select ${r.proforma_invoice_number}`;
              return (
                <tr
                  key={r.id}
                  className={
                ...
                >
                  ...
                  <GroupCell textCenter>
                    <StackLine className="capitalize">{r.invoice_type}</StackLine>
                  </GroupCell>

                  <GroupCell textCenter>
                    <select
                      value={r.proforma_status ?? "pending"}
                      onChange={(e) =>
                        onStatusChange(
                          r,
                  ...
                  <td className="align-top px-2 py-2 text-right">
                    ...
                    >
                      ...
                      <Link
                        href={`${TAX_INVOICE_LIST_PATH}?new=1&proforma_invoice_id=${encodeURIComponent(r.id)}`}
        ...
        <FinanceProformaInvoicesFooterBar
