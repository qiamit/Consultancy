...
export function BisProjectsMasterFooterBar({
  ...
  /** Render as a standalone bar (mobile cards) instead of table footer. */
  asBar?: boolean;
  goPageInputId?: string;
}) {
      ...
      {showPagination ? (
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="flex flex-wrap items-center gap-1.5">
            <label htmlFor={goPageInputId} className={CLIENT_FIELD_LABEL_CLASS}>
              Go to
            </label>
            <input
              id={goPageInputId}
  ...
  if (asBar) {
    return (
      <div className="border-t border-zinc-200 bg-zinc-100 px-3 py-2 text-sm font-medium text-zinc-800 dark:border-zinc-700 dark:bg-zinc-800/90 dark:text-zinc-200">
        {inner}
      </div>
    );
  }
