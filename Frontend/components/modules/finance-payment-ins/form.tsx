...
import { DROPDOWN_KEY_FINANCE_PAYMENT_IN_CLIENT } from "@backend/shared/dropdown-keys";
import type { AppDropdownOptionRow } from "@backend/shared/types/app-dropdown-option";
import {
  saveFinancePaymentIn,
  deleteFinancePaymentInForm,
} from "@backend/actions/finance-payment-ins";
...
<table style="width:100%;border-collapse:collapse;margin-bottom:14px;">
  <tbody>
    <tr><td style="padding:6px 10px;font-size:11px;font-weight:700;width:35%;border:1px solid #e5e7eb;">Client</td><td style="padding:6px 10px;font-size:11px;border:1px solid #e5e7eb;">${esc(client)}</td></tr>
    <tr><td style="padding:6px 10px;font-size:11px;font-weight:700;border:1px solid #e5e7eb;">Amount Received</td><td style="padding:6px 10px;font-size:11px;font-weight:700;border:1px solid #e5e7eb;">${formatInr(amt)}</td></tr>
...
<div style="margin-top:32px;display:flex;justify-content:flex-end;">
  ...
  const printDoc = async () => {
    ...
    openPrintPreview({
      buildDoc: (s, c) =>
        buildPrintDocument({
          title: `Payment IN ${formValues.id || ""}`,
  ...
  const createPdfBlob = async (): Promise<Blob> => {
    ...
    try {
      ...
      return (await html2pdf().from(mount).set({
        margin: [6, 6, 6, 6],
        filename: `payment-in-${formValues.id || "draft"}.pdf`,
  ...
  const downloadPdf = async () => {
    ...
    a.href = url;
    a.download = `payment-in-${formValues.id || "draft"}.pdf`;
    a.click();
  ...
  const sharePdf = async () => {
    try {
      const blob = await createPdfBlob();
      const file = new File([blob], `payment-in-${formValues.id || "draft"}.pdf`, { type: "application/pdf" });
      if (navigator.share && (navigator as Navigator & { canShare?: (d: ShareData) => boolean }).canShare?.({ files: [file] })) {
        await navigator.share({ title: "Payment IN", files: [file] });
        return;
      }
  ...
  return (
    <div className="px-4 pb-5 pt-0">
      <FinanceFormOverlayHeader
        titleId="finance-payment-in-form-title"
        title={formValues.id ? "Edit Payment IN" : "New Payment IN"}
        onClose={onClose}
      ...
      <form action={saveFinancePaymentIn} className="grid gap-3 md:grid-cols-2">
        ...
        <div className="flex flex-wrap items-center justify-end gap-3 border-t border-zinc-200 pt-4 md:col-span-2 dark:border-zinc-800">
          ...
          {formValues.id ? (
            <button
              type="submit"
              formAction={deleteFinancePaymentInForm}
