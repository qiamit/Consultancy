...
import { DROPDOWN_KEY_FINANCE_CREDIT_NOTE_CLIENT } from "@backend/shared/dropdown-keys";
...
export function FinanceCreditNoteForm({
  ...
  <div class="pad grid">
    ...
    <div class="right"><div class="title">PROFORMA INVOICE</div><div class="muted">Date: ${esc(formatDisplayDate(formValues.credit_note_date, "-"))} | Valid until: ${esc(formatDisplayDate(formValues.valid_until_date, "-"))}</div></div>
  </div>
  <div class="section">
  ...
  const printTaxInvoice = async () => {
    ...
    openPrintPreview({
      buildDoc: (s, c) => buildPrintDocument({
        title: `Credit Note ${taxDisplayNumber || ""}`,
  ...
  return (
    <div className={rootClass}>
        <FinanceFormOverlayHeader
          titleId="finance-credit-note-form-title"
          title={
            isNewParam
              ? "New Credit Note"
              : idParam
          ...
          addNewLabel="Add New Credit Note"
          onAddNew={onAddNew}
        />
