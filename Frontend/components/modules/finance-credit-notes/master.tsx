...
import {
  deleteFinanceCreditNote,
  deleteFinanceCreditNotes,
  importFinanceCreditNotes,
  updateFinanceCreditNoteStatus,
} from "@backend/actions/finance-credit-notes";
import {
  buildFinanceCreditNotesExportCsv,
  parseFinanceCreditNotesImportCsv,
} from "@backend/modules/finance/finance-credit-notes-csv";
...
import {
  emptyForm,
  emptyLine,
  CREDIT_NOTE_LIST_PATH,
  rowToForm,
  splitTaxInvoiceNumberForForm,
  type TaxInvoiceFormState,
  type TaxInvoiceLineForm,
} from "./constants";
import { FinanceCreditNoteForm } from "./form";
import { FinanceCreditNotesHeaderBar } from "./header-bar";
import { printFinanceCreditNotesList } from "./print-finance-credit-note-list";
import { filterTaxInvoicesBySearch, PAGE_SIZE_OPTIONS } from "./search-utils";
import { FinanceCreditNotesTable } from "./table";
...
function salesOrderToTaxDraft(
  ...
  return {
    ...
    sales_order_id: q.id,
    proforma_invoice_id: "",
    credit_note_number_prefix: nextNumber.credit_note_number_prefix,
    credit_note_number_value: nextNumber.credit_note_number_value,
    credit_note_date: q.order_date,
...
function getNextTaxInvoiceNumberParts(
  ...
  if (rows.length === 0) {
    return {
      credit_note_number_prefix: preferredPrefixOverride ?? "",
      credit_note_number_value: "00001",
    };
  }
  ...
  const latestParts = splitTaxInvoiceNumberForForm(
    sortedByCreatedAt[0]?.credit_note_number ?? "",
  );
  const preferredPrefix =
    preferredPrefixOverride ?? latestParts.credit_note_number_prefix;

  let maxValue = 0;
  let width = 5;

  for (const row of rows) {
    const parts = splitTaxInvoiceNumberForForm(row.credit_note_number ?? "");
    if (parts.credit_note_number_prefix !== preferredPrefix) continue;
    if (!/^\d+$/.test(parts.credit_note_number_value)) continue;

    const parsed = Number(parts.credit_note_number_value);
    if (!Number.isFinite(parsed)) continue;

    maxValue = Math.max(maxValue, parsed);
    width = Math.max(width, parts.credit_note_number_value.length);
  }

  const next = String(maxValue + 1).padStart(width, "0");
  return {
    credit_note_number_prefix: preferredPrefix,
    credit_note_number_value: next,
  };
}

export function FinanceCreditNotesMaster({
  initialRows,
  fetchError,
  queryError,
  clientRows,
  ...
  initialRows: FinanceCreditNoteRow[];
  ...
  const [form, setForm] = useRouteBoundFormState<TaxInvoiceFormState>(
    ...
    (prev) => {
      if (isNewParam) {
        const prevPrefix = prev.credit_note_number_prefix.trim();
  ...
  const creditNoteReturnUrl = useMemo(() => {
    const q = new URLSearchParams();
    if (isNewParam) q.set("new", "1");
    else if (idParam) q.set("id", idParam);
    const s = q.toString();
    return s ? `${CREDIT_NOTE_LIST_PATH}?${s}` : CREDIT_NOTE_LIST_PATH;
  }, [isNewParam, idParam]);
  ...
  useEffect(() => {
    ...
    q.delete("client_id");
    const qs = q.toString();
    router.replace(`${CREDIT_NOTE_LIST_PATH}${qs ? `?${qs}` : ""}`, {
      scroll: false,
    });
  ...
  const selectRow = useCallback(
    (r: FinanceCreditNoteRow) => {
      router.replace(`${CREDIT_NOTE_LIST_PATH}?id=${encodeURIComponent(r.id)}`, {
        scroll: false,
      });
    },
    [router],
  ...
  const downloadRow = useCallback((r: FinanceCreditNoteRow) => {
    ...
    a.href = url;
    a.download = `${(r.credit_note_number || "credit-note").trim()}.json`;
    a.click();
    URL.revokeObjectURL(url);
  }, []);

  const shareRow = useCallback(
    async (r: FinanceCreditNoteRow) => {
      const url = `${window.location.origin}${CREDIT_NOTE_LIST_PATH}?id=${encodeURIComponent(r.id)}`;
      const text = `Credit Note ${r.credit_note_number || r.id}`;
      try {
        if (navigator.share) {
          await navigator.share({ title: "Credit Note", text, url });
          return;
        }
  ...
  const updateRowStatus = useCallback(
    async (
      ...
      setRows((prev) =>
        prev.map((x) => (x.id === r.id ? { ...x, credit_note_status: status } : x)),
      );
      const result = await updateFinanceCreditNoteStatus({ id: r.id, status });
      if (!result.ok) {
        setRows((prev) =>
          prev.map((x) =>
            x.id === r.id
              ? { ...x, credit_note_status: r.credit_note_status ?? "pending" }
              : x,
          ),
        );
  ...
  const addNew = useCallback(() => {
    router.replace(`${CREDIT_NOTE_LIST_PATH}?new=1`, { scroll: false });
  }, [router]);

  const closeForm = useCallback(() => {
    router.replace(CREDIT_NOTE_LIST_PATH, { scroll: false });
  }, [router]);

  const updateField = useCallback((key: keyof TaxInvoiceFormState, value: string) => {
    setForm((f) => {
      if (key === "invoice_type") {
  ...
  const updateLine = useCallback((index: number, patch: Partial<TaxInvoiceLineForm>) => {
    setForm((f) => {
  ...
  const onTaxDateChange = useCallback((iso: string) => {
    setForm((f) => ({
      ...f,
      credit_note_date: iso,
      valid_until_date: addOneMonthISO(iso),
    }));
  }, []);

  function handleExport() {
    const csv = buildFinanceCreditNotesExportCsv(filtered);
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = "finance-credit-notes-export.csv";
    a.click();
    URL.revokeObjectURL(url);
  }
    ...
    if (toPrint.length === 0) {
      window.alert(
        selectedIds.size > 0
          ...
          : "No rows to print. Adjust your search or add credit notes.",
      );
      return;
    }
    printFinanceCreditNotesList(toPrint);
  }

  async function handleImportFile(file: File) {
    const text = await file.text();
    const parsed = parseFinanceCreditNotesImportCsv(text);
    if (!parsed.ok) {
      window.alert(parsed.error);
      return;
    }
    const result = await importFinanceCreditNotes(parsed.rows);
    if (!result.ok) {
      window.alert(result.error);
      return;
    }
    window.alert(`Imported ${result.inserted} credit note(s).`);
    router.refresh();
  }
  ...
  function handleDelete() {
    ...
    if (bulkIds.length > 0) {
      ...
      if (
        !window.confirm(
          n === 1
            ? "Delete this credit note permanently? This cannot be undone."
            : `Delete ${n} credit notes permanently? This cannot be undone.`,
        )
      ) {
        return;
      }
      void deleteFinanceCreditNotes(bulkIds);
      return;
    }
    ...
    void deleteFinanceCreditNote(idParam);
  }
  ...
  const errMsg =
    queryError === "dates"
      ? "Credit Note date and due-date date are required."
      : queryError === "type"
        ? "Invalid invoice type."
        : queryError === "lines"
          ? "Add at least one line with quantity, rate, or product."
          : queryError === "credit_note_number_required"
            ? "Credit Note number is required."
            : queryError === "credit_note_number_duplicate"
              ? "That tax number is already in use. Choose a different number."
              : queryError === "db"
    ...
    <div className="w-full max-w-none space-y-0">
      ...
      {formVisible ? (
        <FinanceFormModalShell
          ariaLabelledBy="finance-credit-note-form-title"
          onClose={closeForm}
        >
            <FinanceCreditNoteForm
              visible
              overlay
              formValues={form}
              ...
              onTaxDateChange={onTaxDateChange}
              creditNoteReturnUrl={creditNoteReturnUrl}
