...
function clientLabel(r: FinanceCreditNoteRow): string {
  const c = r.clients;
  if (!c) return "—";
...
function listTableHtml(rows: FinanceCreditNoteRow[]): string {
  const head =
    "<thead><tr><th>#</th><th>Credit Note #</th><th>Date</th><th>Valid until</th><th>Linked SO</th><th>Client</th><th>Type</th><th class=\"num\">Total</th></tr></thead>";
  const body = rows
    .map(
      (r, i) =>
        `<tr><td>${i + 1}</td><td class="mono">${esc(r.credit_note_number)}</td><td>${esc(formatDisplayDate(r.credit_note_date))}</td><td>${esc(formatDisplayDate(r.valid_until_date))}</td><td class="mono">${esc(r.finance_sales_orders?.sales_order_number ?? "—")}</td><td>${esc(clientLabel(r))}</td><td>${esc(r.invoice_type)}</td><td class="num">${esc(
...
function openPrintWindow(html: string): void {
  const existing = document.getElementById("finance-credit-notes-print-root");
  existing?.remove();

  const root = document.createElement("div");
  root.id = "finance-credit-notes-print-root";
  root.setAttribute(
    "style",
    [
      "position:fixed",
...
/** Print selected / filtered credit notes — tabular list. */
export function printFinanceCreditNotesList(rows: FinanceCreditNoteRow[]): void {
