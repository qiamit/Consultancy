...
export function IsCodeMasterHeaderBar({
  onAddNew,
  searchQuery,
  onSearchChange,
  pageSize,
  ...
  return (
    <header className="border-b border-zinc-200 bg-zinc-50/90 px-4 py-3 dark:border-zinc-800 dark:bg-zinc-900/80">
      <div className="flex flex-wrap items-center gap-x-3 gap-y-2.5">
        ...
        <div className="flex min-w-[12rem] flex-1 basis-[14rem] items-center gap-2 sm:max-w-sm">
          <label htmlFor="is-code-master-search" className="sr-only">
            Search all fields: IS number, title, aspect, unit, slabs, files,
            and more
          </label>
          <input
            id="is-code-master-search"
            type="search"
            value={searchQuery}
            onChange={(e) => onSearchChange(e.target.value)}
            placeholder="Search All Fields"
            autoComplete="off"
            className="min-w-0 flex-1 rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-900 shadow-sm outline-none placeholder:text-zinc-400 focus:border-sky-500 focus:ring-2 focus:ring-sky-500/30 dark:border-zinc-600 dark:bg-zinc-950 dark:text-zinc-100 dark:placeholder:text-zinc-500"
          />
          <select
            id="is-code-master-page-size"
            value={pageSize}
        ...
        {showPagination ? (
          <div className="flex flex-wrap items-center gap-x-2 gap-y-1.5">
            <p className="whitespace-nowrap text-xs text-zinc-600 dark:text-zinc-400">
              Page{" "}
            ...
            <div className="flex items-center gap-1.5">
              <label
                htmlFor="is-code-master-go-page"
                className={CLIENT_FIELD_LABEL_CLASS}
              >
                Go to
              </label>
              <input
                id="is-code-master-go-page"
                type="number"
                min={1}
                max={totalPages}
                value={goDraft}
        ...
        <div className="ml-auto flex shrink-0 items-center gap-2">
          <button
            type="button"
            className="inline-flex items-center justify-center whitespace-nowrap rounded-lg border border-violet-400 bg-violet-50 px-3 py-2 text-sm font-medium text-violet-700 shadow-sm hover:bg-violet-100 dark:border-violet-700 dark:bg-violet-950/40 dark:text-violet-300 dark:hover:bg-violet-900/40"
            title="Open QE Assistant — AI-powered Quality Engineering helper"
            onClick={() =>
              window.dispatchEvent(
                new CustomEvent("qe-assistant:open", {
                  detail: { module: "is-codes" },
                }),
              )
            }
          >
          ...
          <button
            type="button"
            onClick={onAddNew}
            className="inline-flex items-center justify-center whitespace-nowrap rounded-lg bg-sky-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-sky-500"
          >
            Add New IS Code
          </button>
        </div>
