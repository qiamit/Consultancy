...
function listTableHtml(rows: FinanceSalesOrderRow[]): string {
  const head =
    "<thead><tr><th>#</th><th>Sales order #</th><th>Date</th><th>Expected delivery</th><th>Linked QT</th><th>Client</th><th>Type</th><th class=\"num\">Total</th></tr></thead>";
  const body = rows
    .map(
      (r, i) =>
        `<tr><td>${i + 1}</td><td class="mono">${esc(r.sales_order_number)}</td><td>${esc(formatDisplayDate(r.order_date))}</td><td>${esc(formatDisplayDate(r.expected_delivery_date))}</td><td class="mono">${esc(r.finance_quotations?.quotation_number ?? "—")}</td><td>${esc(clientLabel(r))}</td><td>${esc(r.order_type)}</td><td class="num">${esc(
...
function openPrintWindow(html: string): void {
  const existing = document.getElementById("finance-sales-orders-print-root");
  existing?.remove();

  const root = document.createElement("div");
  root.id = "finance-sales-orders-print-root";
  root.setAttribute(
    "style",
    [
      "position:fixed",
...
/** Print selected / filtered sales orders — tabular list. */
export function printFinanceSalesOrdersList(rows: FinanceSalesOrderRow[]): void {
