...
import { updateIsoProject } from "@/lib/actions/iso-projects";
import { uploadIsoDocument } from "@/lib/actions/documents";
import { createClient } from "@/lib/supabase/server";

const kinds = [
  { value: "new_certification", label: "New certification" },
  { value: "existing_certification", label: "Existing certification / surveillance" },
];
...
export default async function IsoProjectDetailPage({
  params,
}: {
  ...
  return (
    <div className="mx-auto max-w-2xl space-y-10">
      ...
      <form
        action={updateIsoProject.bind(null, id)}
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            ...
            <input
              ...
              defaultValue={row.accrediting_body ?? ""}
              className="block w-full rounded-lg border border-zinc-300 bg-white px-3 py-2 text-sm shadow-sm outline-none focus:border-sky-500 focus:ring-2 focus:ring-sky-500/30 dark:border-zinc-700 dark:bg-zinc-950 dark:text-zinc-100"
            />
          </div>
          <div className="space-y-2">
